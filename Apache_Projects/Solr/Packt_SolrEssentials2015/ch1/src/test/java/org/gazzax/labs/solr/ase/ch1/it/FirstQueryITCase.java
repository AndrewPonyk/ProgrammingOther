...
public class FirstQueryITCase {
	...
	public void myTest() throws SolrServerException, IOException{
		System.out.println();

		// 1 Adding
		// we can define our Model in schema.xml
		final SolrInputDocument input = new SolrInputDocument();
		input.setField("id", "2");
		input.setField("title", "Milan voyage");

		client.add(input);
		client.commit();

// ================================================
		// 2 Getting data from solr
		//"select all" query.
		final SolrQuery query = new SolrQuery("*:*");
		// 5. Executes the query
		final QueryResponse response = client.query(query);
		System.out.println("Result count : " + response.getResults().getNumFound());
		for(SolrDocument item : response.getResults()){
